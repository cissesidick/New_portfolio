const portfolioData = {
    skills: [
        { name: "HTML", category: "frontend", icon: "fab fa-html5", level: 100, emoji: "🔵" },
        { name: "CSS", category: "frontend", icon: "fab fa-css3-alt", level: 90, emoji: "🎨" },
        { name: "JavaScript", category: "frontend", icon: "fab fa-js-square", level: 92, emoji: "📜" },
        { name: "React", category: "frontend", icon: "fab fa-react", level: 80, emoji: "⚛️" },
        { name: "Tailwind CSS", category: "frontend", icon: "fas fa-paint-brush", level: 88, emoji: "🌬️" },
        { name: "Python", category: "backend", icon: "fab fa-python", level: 90, emoji: "🐍" },
        { name: "Django", category: "backend", icon: "fas fa-leaf", level: 85, emoji: "🟢" },
        { name: "MySQL", category: "backend", icon: "fas fa-database", level: 88, emoji: "🐬" },
        { name: "PHP", category: "backend", icon: "fab fa-php", level: 90, emoji: "🔵" },
        { name: "Git", category: "tools", icon: "fab fa-git-alt", level: 90, emoji: "🔄" },
        { name: "GitHub", category: "tools", icon: "fab fa-github", level: 90, emoji: "🐙" },
        { name: "VS Code", category: "tools", icon: "fas fa-code", level: 100, emoji: "📝" },
        { name: "React Native", category: "mobile", icon: "fab fa-react", level: 80, emoji: "📱" },
        { name: "Flutter", category: "mobile", icon: "fab fa-flutter", level: 85, emoji: "📱" }
    ],
    projects: [
        {
            id: 1,
            title: "GESTION DE STOCK",
            description: "Une application complète de gestion de stock permettant de suivre les inventaires, gérer les entrées/sorties et optimiser la chaîne d'approvisionnement en temps réel.",
        ...
        },
        {
            ...
            description: "Application mobile intuitive permettant de localiser rapidement les établissements médicaux (hôpitaux, pharmacies) à proximité. Intègre la géolocalisation et les itinéraires.",
        ...
        },
        {
            ...
            description: "Plateforme web pour la gestion automatisée des présences des employés. Suivi en temps réel, génération de rapports et gestion des absences.",
...
function renderSkills(filter = 'all') {
    ...
    filteredSkills.forEach((skill, index) => {
        ...
        skillCard.className = 'skill-card bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700 hover:shadow-lg hover:shadow-blue-500/20 dark:hover:shadow-blue-500/10 transition-all duration-300';
        skillCard.innerHTML = `
            <div class="flex items-center justify-between mb-3">
                <div class="text-3xl">${skill.emoji}</div>
                <span class="text-sm font-bold text-blue-600 dark:text-blue-400">${skill.level}%</span>
            </div>
            <h3 class="font-semibold text-slate-900 dark:text-white mb-3">${skill.name}</h3>
            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600 to-purple-600 h-full rounded-full transition-all duration-1000 ease-out skill-progress" style="width: 0%" data-width="${skill.level}%"></div>
            </div>
        `;
...
function renderProjects() {
    ...
    allProjects.forEach((project, index) => {
        ...
        projectCard.className = 'project-card flex-shrink-0 w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] group';
        projectCard.innerHTML = `
            <div class="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-lg border border-slate-100 dark:border-slate-700 h-full flex flex-col transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10">
                ...
                <!-- Zone Infos : En dessous pour ne rien cacher -->
...
function updateCarousel() {
    ...
    dots.forEach((dot, index) => {
        if (index === currentSlide) {
            ...
            dot.className = 'w-2 h-2 rounded-full bg-slate-300 dark:bg-slate-600 transition-all';
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    const elements = document.querySelectorAll('.scroll-animate, .skill-card, .project-card, section h2');
    elements.forEach(el => {
...
contactForm.addEventListener('submit', (e) => {
    ...
    submitBtn.innerHTML = '<i class="fas fa-spinner spinning"></i> Envoi en cours...';
    submitBtn.disabled = true;

    setTimeout(() => {
        alert('Merci ! Votre message a été envoyé avec succès. Je vous recontacterai bientôt.');
        contactForm.reset();
